...
The default backend is OpenCV. It does not call an external ``ffmpeg`` command,
although compressed video codecs are still provided by native libraries bundled
...
def video_info(video_path: str) -> dict:
    """Return basic metadata using OpenCV only."""
    import cv2
...
def write_frames(
    ...
    """Write an iterable of BGR frames to a video using OpenCV."""
    import cv2
...
def extract_frames(
    ...
    """Extract frames to image files without invoking an external video command."""
    import cv2
...
def images_to_video(
    ...
    """Create a video from image files using OpenCV."""
    import cv2
